...
```{r eval=FALSE, include=FALSE}
fig_evo_strategy_sporadic = ggplot(df_strat)+
  geom_col(
    aes(
      gen, prop,
      fill = social_strat
...
![**No evolutionary change in social movement strategies when novel pathogen introduction events are relatively uncommon.** In our alternative implementation of the probability with which the infectious pathogen is introduced in each generation, there is no shift in the mixture of movement strategies after pathogen introduction. The agent tracking strategy remains common across parameter combinations.](figures/fig_evo_strategy_sporadic.png){ width=70% }
