...
### Visualising the frequency of social movement strategies

Assign social movement strategies using the function `get_social_strategy`.
...
The `get_social_strategy` function does not require assignment, as it uses data.table's _modify by reference_ feature.

```{r echo=TRUE, eval=TRUE}
# assign social movement strategy
get_social_strategy(pop_data)

# count social strategy by generation
data_social_movement = pop_data[, .N, by = c("gen", "social_strat")]
```

Plot frequency of social movement strategies.

```{r echo=TRUE, eval=TRUE}
# plot as a stacked bar chart
ggplot(data_social_movement) +
  geom_col(
    aes(
      gen, N,
      fill = social_strat
...
The exploration of ecological outcomes is not shown here, as this is a very standard data transformation and visualisation procedure.
