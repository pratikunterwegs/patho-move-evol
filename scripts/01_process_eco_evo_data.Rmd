...
for (i in files) {
  ...
  handling_time <- ifelse(is.null(data$params$handling_time),
    NA_real_, data$params$handling_time
  )
  ...
  p_v_transmit <- ifelse(is.null(data$params$p_v_transmit),
    NA_real_, data$params$p_v_transmit
  )
    ...
    lapply(
      ...
      function(df) {
        df[, c(
          ...
          "p_v_transmit", "handling_time",
          "scenario", "scenario_tag"
        ) := list(
          repl, cost, regen, dispersal, infect_percent, p_v_transmit,
          handling_time, scenario, scenario_tag
        )]
      }
    )
  )
...
b <- a[, list(moved = mean(moved)), by = c("gen", "social_strat")]
ggplot(b, aes(gen, moved, col = social_strat)) +
  geom_line(size = 0.1)
```
